...
const MantrasLibrary = () => {
  ...
  const [speakingMantraId, setSpeakingMantraId] = useState<number | null>(
    null
  );
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <Navbar />
      <PageHeader
        title={decodedCategory || "मंत्र संग्रह"}
        subtitle="वैदिक परंपरा के अनुसार पवित्र संस्कृत मंत्र"
      />
      <main className="flex-grow container mx-auto px-4 py-12 md:py-16">
        ...
        {isError && !isLoading && (
          <div className="text-center text-destructive font-devanagari p-8">
            मंत्र लोड करने में त्रुटि हुई।
          </div>
        )}

        {!isLoading && !isError && filteredMantras.length > 0 && (
          <Card className="max-w-5xl mx-auto p-8 md:p-12 temple-shadow bg-card/50 backdrop-blur-sm">
              ...
              {filteredMantras.map((mantra) => (
                ...
                >
                  <div className="absolute top-4 right-0 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <Button
                      onClick={() =>
                        handleRecite(mantra.content || "", mantra.id)
                      }
                      size="sm"
                      variant="outline"
                    ...
                    <Button
                      onClick={() =>
                        handleCopy(mantra.content || "", mantra.id)
                      }
                      size="sm"
                      variant="outline"
                      className="font-devanagari hover:bg-primary hover:text-primary-foreground transition-all border-2"
        ...
        {!isLoading && !isError && filteredMantras.length === 0 && (
          <div className="text-center text-muted-foreground font-devanagari p-8">
            {decodedCategory
              ? `श्रेणी '${decodedCategory}' में कोई मंत्र नहीं मिला।`
              : "कोई मंत्र उपलब्ध नहीं है।"}
          </div>
        )}
